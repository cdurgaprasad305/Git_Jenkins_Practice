...
class basepage:
        ...
        with allure.step("Opening browser : " + browsername):
            """if self.prod["GridRun"] == constants.GRIDRUN_Y:
                if browsername == constants.CHROME:
                    ...
                    caps["javascriptEnabled"] = True
                try:
                    self.driver = webdriver.Remote(desired_capabilities=caps, command_executor="http://192.168.0.101:4444/wd/hub")
                except Exception as e:
                    print("------Exception in Open_browser" ,e) """
            # else:
            if browsername == constants.CHROME:
    ...
    def takescreenshot(self):
        allure.attach(
            ...
            "Screenshot at : " + str(datetime.now()),
            AttachmentType.PNG,
        )
    ...
    def iselementvisible(self, obj):
        ...
        if obj.endswith("_xpath"):
            ...
            elementlist = wait.until(
                EC.visibility_of_all_elements_located((By.NAME, element))
            )
        elif obj.endswith("_cssSelector"):
            elementlist = wait.until(
                EC.visibility_of_all_elements_located((By.CSS_SELECTOR, element))
            )
        else:
            elementlist = wait.until(
    ...
    def validatetitle(self):
        ...
        if actualtitle == expectedtitle:
            ...
            self.logging(
                "Title validation failed..Got title as "
                + actualtitle
                + " instead of "
                + expectedtitle
            )
    ...
    def clickleadname(self, name):
        ...
        element = self.driver.find_element_by_xpath(
            self.prod["leadnamepart1_xpath"]
            + str(rnum)
            + self.prod["leadnamepart2_xpath"]
        )
        element.click()
    ...
    def selectdate(self, date):
        with allure.step("Selecting date : " + date):
            ...
            while True:
                displayed_date = self.driver.find_element_by_xpath(
                    "//*[@id='calenDiv']/div/div[1]/div/span[3]"
                ).text
