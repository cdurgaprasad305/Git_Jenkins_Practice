...
def getcelldata(testcasename, path):
    ...
    teststartrowindex = 0
    while not (
        xls.getCellData(constants.DATASHEET, teststartrowindex, 0) == testcasename
    ):
    ...
    try:
        while not (
            xls.checkemptycell(constants.DATASHEET, datastartrowindex + maxrow, 0)
        ):
            maxrow = maxrow + 1
    except IndexError:
        pass
...
def isrunnable(testcasename, path):
    ...
    for rNum in range(0, rows):
        ...
        if tname.strip().lower() == testcasename.strip().lower():
