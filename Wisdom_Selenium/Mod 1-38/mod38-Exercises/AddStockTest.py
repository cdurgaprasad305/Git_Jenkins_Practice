...
def ClickandWait(xpathExpTarget, xpathExpWait, maxTime):
    for i in range(0, maxTime):
        ...
        if (
            isElementPresent(xpathExpWait)
            and driver.find_element_by_xpath(xpathExpWait).is_displayed()
        ):
            return
        else:
            time.sleep(1)
        ...
        if desiredMonthYear == displayedMonthYear:
            ...
            if dt > CurrentDate:
                driver.find_element_by_xpath(
                    "//*[@id='datepicker']/table/tbody/tr[1]/td[4]/button"
                ).click()
            elif dt < CurrentDate:
                driver.find_element_by_xpath(
                    "//*[@id='datepicker']/table/tbody/tr[1]/td[2]/button"
                ).click()
...
ClickandWait(
    xpathExpTarget="//*[@id='createPortfolio']/img",
    xpathExpWait="//*[@id='createPortfolioButton']",
    maxTime=10,)
